  ...
  const response = await fetch(modelConfig.apiUrl || 'https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: modelConfig.systemPrompt || '你是一个专业的翻译器，请将以下HTML内容翻译成中文，保持原始的HTML标签不变，只翻译文本内容。'
        },
        {
          role: 'user',
          content: text
...
async function translateWithAzure(modelConfig, text) {
  ...
  const response = await fetch(`${modelConfig.apiUrl}/openai/deployments/${deploymentId}/chat/completions?api-version=${apiVersion}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
async function translateWithGemini(modelConfig, text) {
  const response = await fetch(modelConfig.apiUrl || `https://generativelanguage.googleapis.com/v1beta/models/${modelConfig.model || 'gemini-pro'}:generateContent?key=${modelConfig.apiKey}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
    },
      ...
      contents: [
        {
          parts: [
            {
              text: `${modelConfig.systemPrompt || '你是一个专业的翻译器，请将以下HTML内容翻译成中文，保持原始的HTML标签不变，只翻译文本内容。'}\n\n${text}`
            }
          ]
        }
      ],
...
async function translateWithCustomAPI(modelConfig, text) {
  const response = await fetch(modelConfig.apiUrl, {
    ...
    body: JSON.stringify({
      ...
      prompt: `${modelConfig.systemPrompt || '你是一个专业的翻译器，请将以下HTML内容翻译成中文，保持原始的HTML标签不变，只翻译文本内容。'}\n\n${text}`,
      temperature: parseFloat(modelConfig.temperature) || 0.3
    })
  });
